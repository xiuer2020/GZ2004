...
import {CHECK_MUSIC, GET_MUSIC_DETAIL, GET_MUSIC_LYRIC, GET_MUSIC_URL} from "../../api";
...
class PlayView extends Component {
    ...
    constructor(props) {
        ...
        this.onProgressHandle = this.onProgressHandle.bind(this)
    }


    async checkMusic() {
        let {id} = this.props.match.params
        let data = await CHECK_MUSIC(id)
        if (data.success) {
            Toast.loading('loading', 0)
            this.getPlayViewData()
        } else {
    ...
    getSecond(m, s) {
        return Math.floor((m * 60 + Number(s)) * 100) / 100
    }

    async getPlayViewData() {
        let {id} = this.props.match.params
        let url = await GET_MUSIC_URL(id)
        console.log(url.data[0])
        let detail = await GET_MUSIC_DETAIL(id)
        console.log(detail.songs[0])
        let lyric = await GET_MUSIC_LYRIC(id)
        ...
        // 对歌词数组进行解析
        // 先获取没有时间只有歌词的数组
        let lyricArr = lyric.map(l => l.split(']')[1])
        console.log(lyricArr)

        // 获取每一句歌词对应时间的数组
        let lyricTimeArr = lyric.map(l => {
            // 使用组匹配 获取字符串[(分钟):(秒数)]
            // 分钟 等于 RegExp.$1
            // 秒数 等于 RegExp.$2
            /\[(\d+):(\d+\.\d+)]/ig.test(l)
            return this.getSecond(RegExp.$1, RegExp.$2)
        })
    ...
    // 播放器正在播放中
    onProgressHandle({playedSeconds}) {
        let {lyricTimeArr} = this.state

        for (let i = 0; i < lyricTimeArr.length; i++) {
            // console.log(i, playedSeconds, lyricTimeArr[i], lyricTimeArr[i + 1])
            if (i >= lyricTimeArr.length - 1 || (playedSeconds >= lyricTimeArr[i] && playedSeconds < lyricTimeArr[i + 1])) {
                // console.log(i)
                this.setState({
                    lycIndex: i
                })
    ...
    componentDidUpdate(prevProps, prevState) {
        if (prevState.lyric !== this.state.lyric && this.state.detail) {
            if (this.lrcSrcoll) {
                this.lrcSrcoll.refresh()
            } else {
                this.lrcSrcoll = new BScroll(this.lrc, {
                    disableMouse: true,
                    disableTouch: true
                })
            }
        ...
        // 高亮歌曲下标发生改变,滚动到对应的歌词
        if (prevState.lycIndex !== this.state.lycIndex) {
            this.lrcSrcoll.scrollToElement(`:nth-child(${this.state.lycIndex})`, 300, 0, 32)
        }
    }

    render() {
        const {detail, url, playing, lyricArr, lycIndex} = this.state
        return (
            <div className="play-view">
                    ...
                    detail && (
                        <>
                            <div className="bg-img" style={{
                                backgroundImage: `url(${detail.al.picUrl})`
                            }}/>
                            <div className="music-wrapper" onClick={() => this.playMusic()}>
                                <div className="music-player">
                                    <div className="record-wrapper">
                                        <div className="music-record" style={{
                                            animationPlayState: playing ? 'running' : 'paused'
                                        }}>
                                            <div className="music-record-img" style={{
                                                backgroundImage: `url(${detail.al.picUrl})`,
                                                backgroundSize: 'contain'
                                            }}/>
                                        </div>
                                    </div>
                                    {!playing && <div className="play-btn"/>}
                                </div>
                                <h3 className="music-name">
                                    {detail.name} - <span
                                    className="music-artists">{detail.ar.map(ar => ar.name).join('/')}</span>
                                </h3>
                                <div className="lyric-wrapper" ref={el => this.lrc = el}>
                                    <ul>
                                        {
                                            lyricArr.map((lrc, i) => (
                                                <li className={lycIndex === i ? "active lrc-item" : "lrc-item"} key={i}>
                                                    {lrc}
                                                </li>
                                            ))
                                        }
                ...
                <div className="play-logo">
                    <svg className="u-svg u-svg-logosong" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 184 34">
                        <g fill="#fff">
                            <path
                                d="m78.74 14.27h14.52v-2.27h-14.52v2.27m-1.44 1.89c-.51-.03-.915-.44-.94-.95v-8.21c-.0001-.53.411-.968.94-1h17.37c.529.032.941.47.939 1v8.22c-.023.51-.43.92-.939.95l-17.37-.001m1.44-6h14.52v-2.23h-14.52v2.23"></path>
                            <path
                            ...
                            <path
                                d="m105.42 9.44h17.641c.725-.005 1.309-.595 1.309-1.32v-.66c0-.156-.123-.285-.279-.29h-19.721c-.166 0-.299.134-.299.3 0 .003 0 .007 0 .001v.6c-.006.746.594 1.354 1.34 1.36.0001 0 .005 0 .009 0"></path>
                            <path
                            ...
                            <path
                                d="m54.47 18.15c.045-.074.045-.166 0-.24l-3.61-6.32c-.046-.072-.125-.117-.21-.12h-2.1c-.127-.002-.231.099-.234.226 0 .043.011.086.034.124l3.56 6.18-3.58 6.26c-.061.111-.02.252.092.313.033.018.07.027.108.027h2.1c.084.002.163-.04.21-.109l3.62-6.35"></path>
                            <path
                            ...
                            <path
                                d="m151.12 18c.625 0 1.131.506 1.131 1.13v9.47c0 .625-.506 1.131-1.131 1.131h-16.42c-.621-.006-1.125-.509-1.129-1.131v-9.47c.004-.622.508-1.124 1.129-1.13h16.42m-1.219 4.2v-2.11h-14v2.73h14v-.62m-14 5.441h14v-2.73h-14v2.73"></path>
                            <path
                            ...
                            <path
                                d="m167.05 20.09h-1.649c-.457 0-.881.229-1.131.61-1.129 1.75-2.73 4.88-4.301 7.38-.094.15-.047.348.102.441.051.031.109.049.17.049h1.5c.453.004.877-.227 1.119-.61 1.781-2.819 3.439-5.91 4.391-7.51.066-.114.027-.262-.088-.328-.035-.02-.072-.03-.113-.032"></path>
                            <path
                                d="m183.82 28.08c-1.57-2.5-3.16-5.63-4.301-7.38-.246-.38-.668-.608-1.119-.61h-1.65c-.127-.006-.234.093-.24.219-.002.051.012.1.039.142.951 1.61 2.611 4.69 4.381 7.51.244.387.672.618 1.131.61h1.5c.17.004.313-.131.316-.303.002-.067-.018-.133-.057-.188"></path>
                        </g>
                        <path fill="#dd001b" d="M26.26,0H7.74C3.47,0.011,0.011,3.47,0,7.74v18.52C0.011,30.53,3.47,33.988,7.74,34h18.52
c4.271-0.012,7.729-3.47,7.74-7.74V7.74C33.989,3.47,30.53,0.011,26.26,0"></path>
                        <path fill="#fff"
